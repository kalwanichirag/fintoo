...
import { Fetchexternalholdingdetails, Getuanexits, Sendepfotp, Verifyepfotp } from "../../../FrappeIntegration-Services/services/financial-planning-api/externalApi";
...
function LinkEPF(props) {
  ...
  const fetchValidEpfMembers = async () => {
    try {
      const checkData = await getFamilyMember(user_id);
      let payload = {
      ...
      let epf_valid_members = checkData.data;
      setEpfData(response.data.holding_details);
      if (checkData.status_code === "200" && checkData.data?.length > 0) {
        const all = checkData.data
          .filter(v => !holdingUserIds.includes(v.user_id)).map((v) => ({
            name: v.user_name,
            id: v.user_id,
  ...
  const sendEpfOTP = async () => {
    let data = {
      user_id: epfDropdown.epf_member,
      uan: selectedMember.uan,
    };
    let sendOTP = await Sendepfotp(data);
    return sendOTP;
  };
  ...
  const verifyEPFOTP = async () => {
    ...
    setIsLoading(true);
    setModalType(2);
    let verifyOTP = await Verifyepfotp(reqData)
  ...
  const checkUANValidity = async (uan) => {
    try {
      const response = await Getuanexits(user_id, uan)
  ...
  return (
    <>
      ...
      <div>
        <div>
          <div className="mt-2">
            <h3>STEPS TO AUTO ACCESS YOUR / YOUR MEMBER’S EPF DETAILS</h3>
            <ol
              style={{
                lineHeight: "25px",
              }}
            >
              <li>
                Please make sure you have your UAN [Universal Account Number]
                handy.You can find this number in your salary slip.
              </li>
              <li>
                Once you click this button you will be shown a popup where you
                have to select the member whose EPF you want to retrieve and
                UAN.
              </li>
              <li>Once you submit, you will be asked to provide an OTP.</li>
              <li>
                On successful OTP verification, your EPF details will be
                retrieved.
              </li>
            </ol>
          </div>
              ...
              {epfData.length > 0 &&
                epfData.map((epf) => {
                  return (
                    <tr>
                      <td>
                        {epf.first_name} {epf.last_name}
                      </td>
                      <td>{epf.UAN}</td>
                      <td>{epf.asset_footnote}</td>
                      <td>{epf.last_modified}</td>
                      <td>
                        <a href="#" className="download">
                          <img
                            ng-src={imagePath + "/static/media/Images/delete.svg"}
                            alt="Delete"
                            src={imagePath + "/static/media/Images/delete.svg"}
                            onClick={() => {
                              setEpfID(epf.id);
                              setEpfDeletePopup(true);
                            }}
                          />
                        </a>
                      </td>
