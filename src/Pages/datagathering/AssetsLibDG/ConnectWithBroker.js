...
import { Fetchexternalholdingdetails, UPDATEEXTERNALSTOCKHOLDINGS, deleteexternalholdingdetails} from "../../../FrappeIntegration-Services/services/financial-planning-api/externalApi";
const ConnectWithBroker = (props) => {
  ...
  const getConnectedBroker = async (onload) => {
    try {
      ...
      const fetchdata = await Fetchexternalholdingdetails(payload);
      if (fetchdata["status_code"] == 200) {
        setBrokerData([]);
        setEquityHoldingsData(fetchdata["data"]["holding_details"]);
        dispatch({
          type: "ASSETS_UPDATE",
          payload: true,
        });
    ...
    try {
      ...
      if (r.body.fiRequestStatus === "READY") {
        ...
        if (newDat.retryCount < 5) {
          setTimeout(
            () => FIStatus({ ...newDat, retryCount: newDat.retryCount + 1 }),
            15000
          );
        } else {
          toastr.options.positionClass = "toast-bottom-left";
  ...
  const FIPfmDataReport = async (newDat) => {
    try {
      let customHeaders = {
        "Content-Type": "application/json",
        Accept: "application/json",
      ...
      if (r.body.status == "ACTIVE") {
        try {
          ...
          const updated_flag = newDat.updateFlag;

          var userProfile = userData.body[0].fiObjects;
          let customerId = userData.body[0].custId
          var userpanExist = userProfile.filter(
            (v) => v.Profile.Holders.Holder[0].pan == pan
          );
          if (userpanExist.length > 0) {
            var holdings = userpanExist.flatMap(
              (v) => v.Summary.Investment.Holdings.Holding
            );
          } else {
            var holdings = [];
          }
          ...
          if (updated_flag == 1) {
            for (let i = 0; i < userProfile.length; i++) {
              ...
              if (profile.Holders.Holder[0].pan == pan) {
                ...
                var holdings =
                  userProfile[i].Summary.Investment.Holdings.Holding;
                var holdings_temp = holdings.map((item) => {
                  return {
                    ...
                    lastTradedPrice: item.lastTradedPrice,
                    retirement_date:
                      moment(retirement_date).format("YYYY-MM-DD"),
                    pan: pan,
                    mobile: mobile,
                    updateFlag: 1,
                  };
          ...
          let stock_payload = {
            ...
            "holding_consent_id":consent_id,
            "holding_consent_handle_id":consent_handle,
            "holding_daterange_from":daterange_from,
            "holding_daterange_to":daterange_to,
          ...
          gatewayInstance
            ...
            .then(async function (resp) {
              ...
              if (check_asset_flag["data"] == "0") {
                ...
                continuefetching(
                  props.session.data.fp_user_id,
                  resp.smallcaseAuthToken
                );
              }
            })
  ...
  const unlinkyourholdings = async (data) => {
    try {
      let payload = {
        ...
        // user_id: getParentUserId(),
        holding_type: "Stocks",
        holding_id: data.user_specific_id
      };

      const fetchdata = await deleteexternalholdingdetails(payload);
      if (fetchdata["status_code"] == 200) {
        setIsLoading(false);
        ...
        toastr.options.positionClass = "toast-bottom-left";
        scrollToTop();
        toastr.success(fetchdata["message"]);
      } else {
        toastr.options.positionClass = "toast-bottom-left";
        toastr.error(fetchdata["message"]);
      }
    } catch (e) {
      console.log(e);
    }
  ...
  const deleteHolding = () => {
    setIsLoading(true);
    disconnectbroker(
      brokerId.fp_member_id,
      brokerId.fp_holding_auth_id,
      brokerId.fp_holding_id
    );
  };
  ...
  const deletelinkedHolding = () => {
    setIsLoading(true);
    unlinkyourholdings({
      user_specific_id: unlinkId.user_specific_id,
      user_id: unlinkId.user_id,
      // fp_log_id: unlinkId.fp_log_id,
    });
  };
    ...
    <div style={{ overflow: "auto" }}>
      ...
      <table className="bgStyleTable text-center">
        <tbody>
          ...
          (equityHoldingsData && equityHoldingsData.length > 0) ? (
            <>
              {brokerData.map((broker) => (
                <tr>
                  ...
                  {broker.fp_holding_created_datetime != null ? (
                    <td>
                      {moment(broker.fp_holding_created_datetime).format(
                        "DD/MM/YYYY"
                      )}
                    </td>
                  ) : (
                  ...
                  {broker.fp_holding_updated_datetime != null ? (
                    <td>
                      {moment(broker.fp_holding_updated_datetime).format(
                        "DD/MM/YYYY"
                      )}
                    </td>
                  ) : (
              ...
              {equityHoldingsData.map((equity) => (
                <tr>
                  <td>{equity.user_name}</td>
                  <td>Finvu</td>
                  <td>CDSL</td>
                  <td>
                    {moment(equity.holding_created_date).format("DD/MM/YYYY")}
                  </td>
                  <td>
                    {moment(equity.holding_modified_date).format("DD/MM/YYYY")}
                  </td>
                  <td
                    style={{
                      padding: "10px 0",
                  ...
                  >
                    <button
                      ...
                      onClick={() => {
                        setUnlinkID({
                          ...
                          user_specific_id: equity.holding_id,
                    ...
                    <button
                      className="ms-1 default-btn"
                      onClick={() => {
                        refreshData({
                          user_specific_id: equity.holding_id,
                          member_id: equity.user_id,
                          member_name: equity.user_name,
                          user_id: equity.user_id,
                          pan: equity.holding_pan,
                          // fp_log_id: equity.fp_log_id,
                          consent_id: equity.holding_consent_id,
                          consent_handle: equity.holding_consent_handle_id,
                          daterange_from: equity.holding_daterange_from,
                          daterange_to: equity.holding_daterange_to,
                          cust_id: equity.holding_customer_id,
                          // retirement_date: equity.retirement_date,
                          mobile_number: equity.holding_mobile,
                          created_datetime: equity.holding_created_date,
                          updated_datetime: equity.holding_modified_date,
                          updateFlag: 1,
                        });
                      }}
                    >
            ...
            <tr>
              <td colSpan={5}>No broker connected.</td>
            </tr>
          )}
        </tbody>
      </table>
      ...
      <Modal
        className="popupmodal"
        centered
        show={linkholdingsshow}
        onHide={linkHoldingsClose}
      >
        <Modal.Header className="ModalHead">
        ...
        <div className=" p-4 d-grid place-items-center align-item-center">
          <div className=" HeaderModal">
            ...
            >
              <div className="pt-2">
                {" "}
                <b>Are you sure you want to unlink your fetched equities</b>
              </div>
              <div style={{ fontSize: ".9rem" }} className="pt-1">
                Note: Once unlinked you won't be updated on your investment
              </div>
            </div>
          </div>
        </div>
