  ...
  return (
    <>
      <Table responsive className="trx-tbl8">
        ...
        <tbody>
          {(!detailedMfPotfolio?.sip_stp_swp_data ||
            !(detailedMfPotfolio?.sip_stp_swp_data?.sip_list ?? []).length) ? (
            <tr>
              <td colSpan={4}>
            ...
            detailedMfPotfolio.sip_stp_swp_data.sip_list.map((v) => (
                    ...
                    <div className={`${style.SIPButtons}`}>
                      <div className="d-md-block d-none">
                        ...
                        {getItemLocal("family") != 1 && v.cart_deleted == "N" ? (
                          <button
                            onClick={async () => {
                              const isMaintenanceDown =
                                await checkMaintenanceStatus("maintenance");
