...
export default function CibilScoreCard({
  ...
  useEffect(() => {
    const fetchHoldingDetails = async () => {
      try {
        ...
        setScore(fetchedScore ? Number(fetchedScore) : null);
  ...
  const effectiveState = score ? "filled" : state;

  return (
    <div className="glass-card tw-rounded-2xl tw-p-6 tw-pb-5 tw-flex tw-flex-col tw-h-full">
