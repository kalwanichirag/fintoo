...
const TotalSavingss = (props) => {
  const total_saving_next_year = props.nextYearTotalSavings;
  const currentYear = new Date().getFullYear() + 1;
  if (Object.keys(total_saving_next_year).length > 0) {
    const idealCoverObject = total_saving_next_year.find(item => item.name === "Gross Inflow");
    const idealCoverObjectt = total_saving_next_year.find(item => item.name === "Deficit" || item.name === "Savings");
    const idealCoverObjecttt = total_saving_next_year.find(item => item.name === "Gross Outflow");
    ...
    if (idealCoverObjectt) {
      text = total_saving_next_year.find(item => item.name === "Deficit" || item.name === "Savings").name;
    }
    ...
    const [chartOptions, setChartOptions] = useState({
      ...
      title: {
        text: 'Total '+heading_lable+' Yearly ('+currentYear+')'
      },
      xAxis: {
        categories: ['']
      },
