...
function totalCommissionfromUTISpecifiedSecurity() {
    var n = $("#txtUTICommission").val().length > 0 ? parseInt($("#txtUTICommission").val()) : 0
        , t = $("#txtSpecifiedSecurity").val().length > 0 ? parseInt($("#txtSpecifiedSecurity").val()) : 0
        , i = $("#txtNotifiedMutualFund").val().length > 0 ? parseInt($("#txtNotifiedMutualFund").val()) : 0
        , r = $("#txtAgent").val().length > 0 ? parseInt($("#txtAgent").val()) : 0
        , u = $("#txtTotalLICCommission").val().length > 0 ? parseInt($("#txtTotalLICCommission").val()) : 0
        , f = n + t + i + r + u;
    isNaN(f) ? $("#txtTotalAll").val(0) : $("#txtTotalAll").val(f)
    console.log(n, t, i, r, u)
    // $("#txtTotalAll").val(f)
}
function TotalCommissionLIC() {
    var n = $("#txtFirstYear").val().length > 0 ? parseInt($("#txtFirstYear").val()) : 0
        , t = $("#txtRenewalComission").val().length > 0 ? parseInt($("#txtRenewalComission").val()) : 0
        , i = $("#txtFirstYearRenealComissionCombine").val().length > 0 ? parseInt($("#txtFirstYearRenealComissionCombine").val()) : 0
        , r = n + t + i;

    isNaN(r) ? $("#txtTotalLICCommission").val(0) : $("#txtTotalLICCommission").val(r)
    // $("#txtTotalLICCommission").val(r)

}
function CalculateAdHoc() {
    var t = $("#txtFirstYear").val().length > 0 ? parseInt($("#txtFirstYear").val()) : 0
        , i = $("#txtRenewalComission").val().length > 0 ? parseInt($("#txtRenewalComission").val()) : 0
        , r = $("#txtFirstYearRenealComissionCombine").val().length > 0 ? parseInt($("#txtFirstYearRenealComissionCombine").val()) : 0
        , n = .5 * t + .15 * i + .3333 * r;
    n > 2e4 && (n = 2e4);
    var u = $("#txtUTICommission").val().length > 0 ? parseInt($("#txtUTICommission").val()) : 0
        , f = $("#txtSpecifiedSecurity").val().length > 0 ? parseInt($("#txtSpecifiedSecurity").val()) : 0
        , e = $("#txtNotifiedMutualFund").val().length > 0 ? parseInt($("#txtNotifiedMutualFund").val()) : 0
        , o = .5 * (u + f + e);
    isNaN(Math.round(o + n)) ? $("#txtAdhoc").val(0) : $("#txtAdhoc").val(Math.round(o + n))
    // $("#txtAdhoc").val(Math.round(o + n))

}

// $(document).ready(function () {
//     $("#txtFirstYear,#txtRenewalComission,#txtFirstYearRenealComissionCombine").change(function () {
//         // TotalCommissionLIC()
//     });
//     $("#txtNotifiedMutualFund,#txtSpecifiedSecurity,#txtUTICommission,#txtAgent,#txtFirstYear,#txtRenewalComission,#txtFirstYearRenealComissionCombine").change(function () {

//         console.log($('#txtNotifiedMutualFund').val())
//         console.log($('#txtSpecifiedSecurity').val())
//         console.log($('#txtUTICommission').val())
//         console.log($('#txtAgent').val())
//         console.log($('#txtFirstYear').val())
//         console.log($('#txtRenewalComission').val())
//         console.log($('#txtFirstYearRenealComissionCombine').val())

//         // var n, t, i;
//         // totalCommissionfromUTISpecifiedSecurity();
//         // n = $("#txtTotalAll").val().length > 0 ? parseInt($("#txtTotalAll").val()) : 0;
//         // n <= 6e4 ? CalculateAdHoc() : $("#txtAdhoc").val(0);
//         // t = $("#txtAdhoc").val().length > 0 ? parseInt($("#txtAdhoc").val()) : 0;
//         // i = n - t;
//         // isNaN(i) ? $("#txtTaxableCommission").val(0) : $("#txtTaxableCommission").val(i)
//         // $("#txtTaxableCommission").val(i)
//     })
// });
